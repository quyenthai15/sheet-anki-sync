  ...
  for (let i = 0; i < noteIds.length; i += chunkSize) {
    ...
    notesInfoResult.data.forEach((info) => {
      if (info.modelName === model && info.fields[primaryAnkiField]) {
        // Normalize all fields from Anki for consistent Japanese character handling
        const primaryValue = info.fields[primaryAnkiField].value.trim().normalize('NFC');
        const fields = {};
        Object.keys(info.fields).forEach(
          (f) => (fields[f] = info.fields[f].value.trim().normalize('NFC')),
        );

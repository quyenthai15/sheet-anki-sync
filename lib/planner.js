  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (existingNote) {
      if (config.force_sync) {
        // Detect sentence change before building audio (needed to decide whether to clear old tag)
        const sentenceChanged = existingNote && fields[sentenceAnkiField] &&
          existingNote.fields[sentenceAnkiField] !== fields[sentenceAnkiField];
        ...
        // When the sentence changed, clear the SentenceAudio field first to drop the stale tag.
        audio.forEach(a => {
          a.fields.forEach(f => {
            const replacingSentenceAudio = f === config.sentence_audio_field && sentenceChanged;
            const currentVal = replacingSentenceAudio ? "" : (fieldsWithAudio[f] || existingNote.fields[f] || "");
